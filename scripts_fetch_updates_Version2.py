...
OFFICIAL_ORG = os.environ.get("OFFICIAL_ORG", "pinkycollie-official")
...
def classify_repo(repo, official_org):
    ...
    if owner.lower() == official_org.lower():
        return "official"
    # For possible "mirror" feature: add logic if repo is a mirror
    if repo.get("fork"):
        return "fork"
    return "personal"
...
def render_site(updates):
    # Use a simple Jinja2 template for accessible, filterable output.
